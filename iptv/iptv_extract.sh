...
mariadb -N -u root -pP@ssw0rd livetv -e "select mac, validity from providers WHERE provider = 'POWERIP' ORDER BY validity DESC LIMIT 10" > ./iptv/powerip.lst;
mariadb -N -u root -pP@ssw0rd livetv -e "select mac, validity from providers WHERE provider = 'PHOENIX' ORDER BY validity DESC LIMIT 10" > ./iptv/phoenix.lst;
mariadb -N -u root -pP@ssw0rd livetv -e "select mac, validity from providers WHERE provider = 'ULTRABOX' ORDER BY validity DESC LIMIT 10" > ./iptv/ultrabox.lst;

###########################################################################################################################
# UPDATE PROVIDERS FROM SOURCE FILES
###########################################################################################################################
...
while IFS= read -r line; do
	...
	echo 'ON DUPLICATE KEY UPDATE validity = values(validity)' >> ./iptv/valid_providers.temp;
...
WHERE a.provider = b.provider_name' > ./iptv/subscribers_list.temp;
...
while IFS= read -r line; do
	...
	###########################################################################################################################
	# EXTRACT PLAYLIST AND GUIDE DATABASE
	###########################################################################################################################
